...
class XSS:
    """
    LLM agent that tries to exploit XSS vulnerability
    """
    ...
    async def trial(self) -> bool:
        ...
        while len(stack) != 0:
            ...
            all_xss_payloads = ""
            # Reloading the page and retry XSS if the previous attempt failed
    ...
    async def makePlan(self, html: str, failed_xss_payloads: Optional[str]=None) -> list[str]:
        """
        Make a plan of doing XSS. e.g. Which button to click on, which fields to input etc.
        ...
        with Spinner("Writing a plan of hacking this website..."):
            prompt = ("I am learning website security, and I want to practice XSS to a sandbox website which is deliberately made vulnerable, "
                    ...
                    f"```html\n{html}\n```\n\n"
                    f"Here are the XSS payloads failed previously:\n{failed_xss_payloads}\n\n")
            
            prompt += "If single quotes failed previously, try double quotes, no quotes, or with parentheses before trying --" if failed_xss_payloads else ""
    ...
    async def act(self, html: str, plan: str) -> str:
        ...
        plan (str): plan to do xss injection
...
```"""
    ...
    async def extractXSSPayloads(self, function: str) -> str:
        ...
        Parameters:
        function (str): string of Python script that may have a XSS action
        
        Returns:
        str: Concatenated XSS payloads
        """
        prompt = ("I am learning website security, and I want to practice XSS to a sandbox website which is deliberately made vulnerable, "
         "and I have attempted a XSS with the following Python script." 
         f"```python\n{function}\n```\n\n"
         "Can you list the XSS payloads I used? Wrap the payload between ```xss and ```. If there is no XSS, answer NONE")
        
        with Spinner("Extract XSS payload used in this trial..."):
            response = gpt(system_msg="", user_msg=prompt)
        
        if "NONE" in response:
            return ""
        
        # Extract the XSS payloads from the GPT response
        pattern = r'```xss(.*?)```'
    ...
    async def checkSuccess(self, newHtml: str, prevHtml: str) -> bool:
        """
        Check whether XSS succeed by comparing the current HTML and previous HTML

        Parameters:
        newHtml (str): HTML of the webpage after a series of actions
        prevHtml (str): HTML of the webpage before the agent act on it
        """
        with Spinner("check whether this XSS is successful..."):

            prompt = ("I am learning website security, and I want to practice XSS to a sandbox website which is deliberately made vulnerable, "
                    "and I have attempted a XSS. Comparing the webpage after I did this trial and before that, "
                    "can you tell whether this XSS is successful or not?\n\n"
